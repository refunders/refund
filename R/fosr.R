...
##' \url{https://pubmed.ncbi.nlm.nih.gov/21969982/}
##' @examples
##' \dontrun{
##' require(fda)
...
fosr <- function(
  ...
  if (is.null(argvals))
    argvals <- if (is.null(fdobj)) seq(0, 1, length = ncol(Y)) else
      seq(min(fdobj$basis$range), max(fdobj$basis$range), length = 201)
  ...
  if (method != "OLS") {
    ...
    while (
      any(abs((coefmat - coefmat.old) / coefmat.old) > 0.001) &
        (iter < max.iter)
    ) {
      iter = iter + 1
      ...
      coefmat.old = coefmat
      residvec <- as.vector(t(respmat)) -
        (X.sc %x% Bmat) %*% oldfit$coef[1:(q * nbasis)]
      ...
      if (method == "GLS") {
        ...
        if (cov.method == "mod.chol") {
          ...
          for (nband in 1:(p - 1)) {
            ...
            for (k in 2:p) {
              ...
              TT[k, max(1, k - nband):(k - 1)] <- (-qr.coef(
                qrResCent,
                res.cent[, k]
              ))
            ...
            if (nband > 5) {
              if (
                lwstat[nband] > lwstat[1] && lwstat[nband] > lwstat[nband - 1]
              )
                break
            }
          }
          ...
          cat(
            ...
            nband.best,
            "for precision matrix of residuals\n"
          )
          ...
          sqrt.prec <- tcrossprod(
            scale(svd.cov.mle$u, FALSE, sqrt(svd.cov.mle$d)),
            svd.cov.mle$u
          )
        }
        ...
        newfit <- amc(
          as.vector(tcrossprod(sqrt.prec, respmat)),
        ...
        if (resp.type == "fd") {
          ...
          efuncmat.scaled <- Bmat %*%
            t(t(pca.resid$harmonics$coef[, 1:npc]) * sqrt(evalues))
        ...
        if (iter == 1)
          cat(
            "Using leading",
            npc,
            "PCs of residual functions for random effects\n"
          )
        ...
        constr.aug <- if (is.null(constr)) NULL else
          cbind(constr, matrix(0, nrow(constr), npc * ncurve))
        #F
        startB <- if (iter == 1) {
  ...
  if (method == "OLS" | max.iter == 0) {
    ...
    cov_bdiag = new(
      "dgCMatrix",
      Dim = c(M, M),
      i = as.vector(matrix(0L:(M - 1L), nrow = ngrid)[,
        rep(seq_len(ncurve), each = ngrid)
      ]),
      p = ngrid * 0L:M,
      x = rep(as.double(covmat), times = ncurve)
    )
    var.b = as(firstfit$GinvXT %*% cov_bdiag %*% t(firstfit$GinvXT), "matrix")
  ...
  for (j in 1:q) {
    ...
    se.func[, j] = sqrt(rowSums(
      (Theta %*%
        var.b[
          (nbasis * (j - 1) + 1):(nbasis * j),
          (nbasis * (j - 1) + 1):(nbasis * j)
        ]) *
        Theta
    ))
  ...
  yhat = if (resp.type == "raw") X %*% tcrossprod(coefmat, Theta) else
    fd(t(X %*% coefmat), bss)
  llist = list(
    fd = fd,
    pca.resid = pca.resid,
