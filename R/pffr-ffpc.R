...
ffpc <- function(
  ...
  return(list(
    data = xiMat,
    PCMat = klX$efunctions[, 1:min(ncol(klX$scores), npc.max), drop = FALSE],
    meanX = klX$mu,
...
ffpcplot <- function(
  ...
  betadata <- if (object$pffr$is_sparse) {
  ...
  for (i in 1:nterms) {
    ...
    phibeta[[i]] <-
      length(object$pffr$ffpc[[i]]$xind) /
      diff(rev(range(object$pffr$ffpc[[i]]$xind))) *
      object$pffr$ffpc[[i]]$PCMat %*%
