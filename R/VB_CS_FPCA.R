...
#'   \code{beta.LB}), fitted values (\code{Yhat}), and the estimated
#'   residual FPC decomposition (\code{fpca.obj}).
...
vb_cs_fpca = function(
  ...
  while (j < 11) {
    ...
    sigma.q.beta = solve(
      as.numeric((Asig + I * D / 2) / (b.q.sigma.me)) *
        XtX +
    ...
    sigma.q.Bpsi = solve(
      kronecker(diag((Apsi + Kt / 2) / b.q.lambda.Bpsi), P.mat) +
        as.numeric((Asig + J / 2) / (b.q.sigma.me)) *
          f_sum(
            mu.q.c = mu.q.C,
            sig.q.c = sigma.q.C,
            theta = t(Theta),
    ...
    for (subj in 1:I) {
      ...
      mu.q.C[subj, ] = ((Asig + J / 2) / (b.q.sigma.me)) *
        sigma.q.C[[subj]] %*%
          as.matrix(psi.cur[, obs.points]) %*%
          (Y[subj, obs.points] - fixef.cur[subj, obs.points])
    }
