...
#'   \code{beta.LB}), and fitted values (\code{Yhat}).
#' @author Jeff Goldsmith \email{ajg2202@@cumc.columbia.edu}
#' @importFrom splines bs
#' @export
#'
vb_mult_wish = function(
  formula,
  data = NULL,
  verbose = TRUE,
  Kt = 5,
  ...
  for (subj in 1:length(unique(SUBJ))) {
    mu.q.BZ[, subj] = solve(kronecker(Ji[subj], t(Theta) %*% Theta)) %*%
      t(kronecker(rep(1, Ji[subj]), Theta)) %*%
  ...
  Bz = b.q.lambda.BZ = ifelse(
    is.null(Bz),
    .5 *
      sum(diag(
        (t(mu.q.BZ) - Wi %*% t(mu.q.BW)) %*%
          P.mat %*%
          t(t(mu.q.BZ) - Wi %*% t(mu.q.BW))
      )),
    Bz
  )
  ...
  while (
    (j < (min.iter + 2) | (lpxq[j] - lpxq[j - 1]) > 1.0E-1) & (j < max.iter)
  ) {
    ###############################################################
    ...
    for (subj in 1:length(unique(SUBJ))) {
      ...
      sigma.q.BZ[[subj]] = solve(
        t.designmat.Z %*%
          kronecker(diag(1, Ji[subj], Ji[subj]), inv.sig) %*%
          t(t.designmat.Z) +
          ((Az + I * Kt / 2) / b.q.lambda.BZ) * P.mat
      )
      mu.q.BZ[, subj] = sigma.q.BZ[[subj]] %*%
        (t.designmat.Z %*%
          kronecker(diag(1, Ji[subj], Ji[subj]), inv.sig) %*%
          (as.vector(t(Y[which(SUBJ == unique(SUBJ)[subj]), ]))) +
          (((Az + I * Kt / 2) / b.q.lambda.BZ) * P.mat) %*%
    ...
    sigma.q.BW = solve(
      (((Az + I * Kt / 2) / b.q.lambda.BZ) * tWIW) +
        kronecker(diag((Aw + Kt / 2) / b.q.lambda.BW), P.mat)
    )
    mu.q.BW = matrix(
      sigma.q.BW %*%
        (((Az + I * Kt / 2) / b.q.lambda.BZ) * tWI %*% as.vector(mu.q.BZ)),
      nrow = Kt,
      ncol = p
    )
    ...
    mu.q.Psi = Psi +
      ...
      matrix(
        apply(
          sapply(
            1:I,
            function(u) Ji[u] * Theta %*% sigma.q.BZ[[u]] %*% t(Theta)
          ),
          1,
          sum
        ),
    ...
    trace.temp = sapply(
      ...
      function(p)
        sum(diag(
          P.mat %*%
            sigma.q.BW[(Kt * (p - 1) + 1):(Kt * p), (Kt * (p - 1) + 1):(Kt * p)]
        ))
    )
    b.q.lambda.BZ = as.numeric(
      Bz +
        .5 *
          sum(sapply(1:I, function(u) {
            mu.q.BZ[, u] %*%
              P.mat %*%
              mu.q.BZ[, u] +
              sum(diag(P.mat %*% sigma.q.BZ[[u]])) -
    ...
    curlpxq = .5 *
      sum(sapply(1:I, function(u) det(sigma.q.BZ[[u]]))) +
      .5 *
        sum(sapply(
          1:p,
          function(u)
