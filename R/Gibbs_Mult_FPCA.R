...
#'   fitted values (\code{Yhat}), random effects (\code{ranef}), and FPC
#'   basis functions (\code{psi.pm}).
#' @importFrom splines bs
#' @importFrom MASS mvrnorm
#' @export
#'
gibbs_mult_fpca = function(
  formula,
  Kt = 5,
  Kp = 2,
  data = NULL,
  ...
  for (i in 1:N.iter) {
    ...
    sigma = solve(
      (1 / sig2.me) *
        kronecker(t(c.mat) %*% c.mat, t(Theta) %*% Theta) +
        kronecker(diag(1 / lambda.psi), P.mat)
    )
    mu = (1 / sig2.me) *
      sigma %*% t(kronecker(c.mat, Theta)) %*% (Y.vec - mean.cur)
    ...
    for (subj.vis in 1:(IJ)) {
      ...
      mu = (1 / sig2.me) *
        sigma %*%
          psi.cur %*%
          (Y[subj.vis, ] - fixef.cur[subj.vis, ] - ranef.cur[subj.vis, ])
