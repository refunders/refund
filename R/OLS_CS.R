...
#'   functions (\code{beta.hat}), fitted values (\code{Yhat}), the fitted
#'   \code{lm} object (\code{model.ols}), and estimated residual covariance
#'   (\code{sigma}).
...
ols_cs = function(formula, data = NULL, Kt = 5, basis = "bs", verbose = TRUE) {
  ...
  if (verbose) {
    cat("Using OLS to estimate model parameters \n")
  }
