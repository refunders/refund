...
#' Series C}, 65 215-236.
#'
#' @return A list of class \code{"fosr"} containing estimated coefficient
#'   functions (\code{beta.hat}), credible bounds (\code{beta.UB},
#'   \code{beta.LB}), fitted values (\code{Yhat}), random effects
#'   (\code{ranef}), and the estimated residual FPC decomposition
#'   (\code{fpca.obj}).
#' @author Jeff Goldsmith \email{ajg2202@@cumc.columbia.edu}
#' @importFrom splines bs
#' @export
#'
vb_mult_fpca = function(
  formula,
  data = NULL,
  verbose = TRUE,
  Kt = 5,
  ...
  ## argvals
  if (!is.null(argvals)) {
    warning("Argument <argvals> supplied but not used.")
  }
  argvals = seq(0, 1, , D)
  ...
  while (j < 11) {
    ...
    sigma.q.BW = solve(
      as.numeric((A + IJ * D / 2) / (b.q.sigma.me)) *
        sig.X +
    ...
    for (subj in 1:length(unique(SUBJ))) {
      ...
      sig.Z = kronecker(
        t(rep(1, Ji[subj])) %*% rep(1, Ji[subj]),
        t(Theta) %*% Theta
      )

      mean.cur = as.vector(
        fixef.cur[which(SUBJ == unique(SUBJ)[subj]), ] +
      ...
      sigma.q.BZ = solve(
        as.numeric((A + IJ * D / 2) / (b.q.sigma.me)) *
          sig.Z +
          ((A + I * Kt / 2) / b.q.lambda.BZ) * P.mat
      )
      mu.q.BZ[, subj] = sigma.q.BZ %*%
        (as.numeric((A + IJ * D / 2) / (b.q.sigma.me)) *
          t.designmat.Z %*%
            (as.vector(Y[which(SUBJ == unique(SUBJ)[subj]), ]) - mean.cur))
    }
    ...
    sigma.q.Bpsi = solve(
      as.numeric((A + IJ * D / 2) / (b.q.sigma.me)) *
        kronecker(t(mu.q.C) %*% mu.q.C + diag(IJ, Kp, Kp), t(Theta) %*% Theta) +
        kronecker(diag((A + Kt / 2) / b.q.lambda.Bpsi), P.mat)
    )
    mu.q.Bpsi = matrix(
      ((A + IJ * D / 2) / (b.q.sigma.me)) *
        sigma.q.Bpsi %*% (t(designmat) %*% (Y.vec - mean.cur)),
      nrow = Kt,
      ncol = Kp
    )
    ...
    for (subj in 1:IJ) {
      ...
      mu.q.C[subj, ] = ((A + IJ * D / 2) / (b.q.sigma.me)) *
        sigma.q.C[[subj]] %*%
          as.matrix(psi.cur) %*%
          (Y[subj, ] - fixef.cur[subj, ] - ranef.cur[subj, ])
    }
    ...
    b.q.sigma.me = as.numeric(
      B + .5 * crossprod(as.vector(Y - (fixef.cur + ranef.cur + pcaef.cur)))
    )

    ## lambda for fixed effects
    for (term in 1:dim(W.des)[2]) {
      b.q.lambda.BW[term] = B +
        .5 *
          (t(mu.q.BW[, term]) %*%
            P.mat %*%
            mu.q.BW[, term] +
    ...
    b.q.lambda.BZ = as.numeric(
      B +
        .5 *
          (t(vec.BZ) %*%
            kronecker(diag(1, length(Ji)), P.mat) %*%
            vec.BZ +
  ...
  for (i in 1:p) {
    beta.sd[i, ] = sqrt(diag(
      (Theta) %*%
        sigma.q.BW[(Kt * (i - 1) + 1):(Kt * i), (Kt * (i - 1) + 1):(Kt * i)] %*%
        t(Theta)
    ))
  ...
  efunctions = matrix(
    Winvsqrt %*% eigen(V, symmetric = TRUE)$vectors[, seq(len = Kp)],
    nrow = D,
    ncol = Kp
  )
