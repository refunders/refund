...
- **Scheipl, Staicu and Greven (2015)**, *Functional Additive Mixed Models* (JCGS) — the framework `pffr` implements; **Scheipl, Gertheiss and Greven (2016)** (EJS) and **Greven and Scheipl (2017)** (Statistical Modelling) — the modern statements of exactly this approach.
...
1. **Two scalar covariates are entered as functional terms.** `lat` and `lon` are the stations' coordinates — two numbers per station. They are passed as $n\times 365$ matrices, so `pffr` is forced to estimate a **time-varying coefficient with a 50-dimensional basis for each** (`bs.yindex = cr, k=50`), i.e. ~100 spurious parameters for two constants, over 34 training curves per fold. PenFFR receives them as scalars (one coefficient each). This is the dominant cause of `pffr`'s poor Canadian-Weather result.
