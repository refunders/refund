---
title: "pffr CI Benchmark: Gaussian (Round 2) + Non-Gaussian Data + Grid Refinement (Study 2)"
...
- **Non-Gaussian Data (sandwich coverage):** Poisson and Binomial DGPs for
...
The non-Gaussian and Study 2 sections use study-specific summaries defined in
...
- **Gaussian Round 2:** `pffr`, `pffr_hc`, `pffr_sandwich`, `pffr_ar`,
  `pffr_gaulss`.
- **Non-Gaussian:** `default`, `hc`, `cluster`.
- **Study 2:** `default`, `cluster`, `hc`, `cl2`.
...
make_zscore_table <- function(data, term) {
  data |>
    dplyr::filter(term_type == term) |>
    select(-term_type) |>
    gt() |>
    tab_header(
      title = paste("Z-Score Diagnostics:", term),
      subtitle = "Under N(0,1): mean→0, sd→1, kurtosis→0, E[z²]→1. Blue = below, white = on target, red = above."
    ) |>
    fmt_number(
...
for (tt in levels(zscore_summary$term_type)) {
  cat(as.character(gt::as_raw_html(make_zscore_table(zscore_summary, tt))))
  cat("\n\n")
}
```
...
```{r zscore-sd-heatmap, fig.height = 5}
...
The z_sd heatmap reveals the SE calibration pattern across methods.
Default pffr is near-ideal (z_sd ≈ 1.0) for most terms but has z_sd > 1
under correlation (shown in the next panel). HC tracks pffr closely.
The sandwich methods (pffr_sandwich, pffr_gaulss, pffr_ar) have z_sd
slightly above 1 on average, reflecting the inclusion of misspecified
conditions in the marginal average.

## Z-Score SD by Correlation Strength

```{r zscore-by-rho, fig.height = 7}
zscore_by_rho <- d |>
  filter(!is.na(rho_f)) |>
...
The z_sd breakdown by correlation strength is the key diagnostic. At rho=0
(IID), all methods are near z_sd=1. As rho increases, default and HC z_sd
climb sharply (to ~1.5+ at rho=0.9 for the ff term), while pffr_sandwich
and pffr_ar remain close to 1. This mechanistically explains the
coverage collapse under correlation.
...
## ANOVA: Which DGP Parameters Drive SE Calibration?

Before looking at individual effect plots, we quantify the relative
importance of each DGP parameter for SE calibration (z_sd) using
Type II ANOVA with eta-squared effect sizes. Two separate ANOVAs
target different subsets where methods are well-specified.

**Caveat**: Observations within the same simulation replicate are correlated
(shared model fit across term types); eta² values are descriptive variance
decompositions, not formal inferential tests.


```{r anova-setup}
old_contrasts <- options(contrasts = c("contr.sum", "contr.poly"))$contrasts
on.exit(options(contrasts = old_contrasts), add = TRUE)

d_anova <- d |>
  mutate(
    method_f = method,
    term_type_f = term_type
  )
```


### ANOVA 1: IID Gaussian — Method × Heteroskedasticity × SNR × Wiggliness × n

Subset: IID + Gaussian errors. Methods: pffr, pffr_sandwich, pffr_hc (all
well-specified under homoskedastic IID). This isolates heteroskedasticity,
SNR, wiggliness, and sample size effects on SE calibration.


```{r anova-iid-gaussian}
d_anova1 <- d_anova |>
  dplyr::filter(
    corr_type == "iid",
    error_dist == "gaussian",
    method %in% c("pffr", "pffr_sandwich", "pffr_hc")
  ) |>
  mutate(method_f = droplevels(method_f))

if (nrow(d_anova1) > 50) {
  fit_zsd_1 <- lm(
    z_sd ~ (method_f + hetero_f + snr_f + wiggliness_f + n_f)^2 + term_type_f,
    data = d_anova1
  )
  aov_1 <- car::Anova(fit_zsd_1, type = "II")
  eta_1 <- effectsize::eta_squared(aov_1, partial = FALSE, ci = 0.95)

  eta_1_df <- eta_1 |>
    as_tibble() |>
    dplyr::filter(Eta2 >= 0.01) |>
    arrange(desc(Eta2))
  ...
  if (nrow(eta_1_df) > 0) {
    eta_1_df |>
      gt() |>
      tab_header(
        title = "ANOVA 1: z_sd under IID Gaussian (pffr, sandwich, HC)",
        subtitle = "Type II eta² (≥ 0.01). Dependent variable: z_sd."
      ) |>
      fmt_number(columns = c(Eta2, CI, CI_low, CI_high), decimals = 3)
  } else {
    cat("All eta² < 0.01 — no single parameter dominates z_sd under IID Gaussian.\n")
  }
} else {
  cat("Insufficient data for ANOVA 1 (IID Gaussian subset).\n")
}
```


### ANOVA 2: Correlation — Method × Correlation × SNR × Wiggliness × n

Subset: homoskedastic Gaussian. Methods: pffr, pffr_sandwich (the two
methods available across all correlation structures). This isolates the
correlation structure effect and its interaction with method.
...
d_anova2 <- d_anova |>
  dplyr::filter(
    hetero_type == "none",
    error_dist == "gaussian",
    method %in% c("pffr", "pffr_sandwich")
  ) |>
  mutate(method_f = droplevels(method_f))

if (nrow(d_anova2) > 50) {
  fit_zsd_2 <- lm(
    z_sd ~ (method_f + corr_f + snr_f + wiggliness_f + n_f)^2 + term_type_f,
    data = d_anova2
  )
  aov_2 <- car::Anova(fit_zsd_2, type = "II")
  eta_2 <- effectsize::eta_squared(aov_2, partial = FALSE, ci = 0.95)
  ...
  eta_2 |>
    ...
    gt() |>
    tab_header(
      title = "ANOVA 2: z_sd under Homoskedastic Gaussian (pffr vs sandwich)",
      subtitle = "Type II eta² (≥ 0.01). Dependent variable: z_sd."
    ) |>
    fmt_number(columns = c(Eta2, CI, CI_low, CI_high), decimals = 3)
} else {
  cat("Insufficient data for ANOVA 2 (correlation subset).\n")
}

options(contrasts = old_contrasts)
```

**ANOVA 1** (IID Gaussian): Under well-specified IID conditions, no single DGP
parameter dominates SE calibration — all eta² values are below 1%. The methods
are well-calibrated in this easy regime.

**ANOVA 2** (Homoskedastic Gaussian): Correlation structure and the method
choice are the strongest drivers of z_sd, followed by term type and the
method × correlation interaction (confirming that methods respond differently
to within-curve dependence). See the table above for exact eta² values.

## Correlation Effect {.tabset}

**Conditioned on**: hetero = none, error = gaussian (all methods well-specified
for the variance structure). Shows: pffr, pffr_sandwich, pffr_hc, pffr_ar.

### Coverage by Correlation
...
corr_summary <- d |>
  dplyr::filter(
    hetero_type == "none",
    error_dist == "gaussian",
    method %in% c("pffr", "pffr_sandwich", "pffr_hc", "pffr_ar")
  ) |>
  summarize_coverage(method, term_type, corr_f)

plot_parameter_effect(corr_summary, "corr_f", "Correlation Structure") +
  labs(
    title = "Effect of Correlation on Coverage",
    subtitle = "Conditioned: hetero=none, error=gaussian. Only cluster-robust handles within-curve dependence."
  )
```
...
```{r corr-effect-table}
make_effect_table(corr_summary, "corr_f", "Correlation")
```

Under homoskedastic Gaussian errors, correlation is the dominant factor.
Default pffr drops from 91% (IID) to 71% (AR1) to 63% (Fourier+).
The cluster sandwich (pffr_sandwich) is far more stable: 88→84→82%.
HC tracks default closely — it corrects heteroskedasticity but not
within-curve dependence. pffr_ar reaches 87% under AR1 and 82% under
Fourier+.

## Heteroskedasticity Effect {.tabset}

**Conditioned on**: corr = IID, error = gaussian (isolates heteroskedasticity).
Shows: pffr, pffr_sandwich, pffr_hc, pffr_gaulss.

### Coverage by Heteroskedasticity

```{r hetero-effect-plot}
hetero_summary <- d |>
  dplyr::filter(
    corr_type == "iid",
    error_dist == "gaussian",
    method %in% c("pffr", "pffr_sandwich", "pffr_hc", "pffr_gaulss")
  ) |>
  summarize_coverage(method, term_type, hetero_f)

plot_parameter_effect(hetero_summary, "hetero_f", "Heteroskedasticity") +
  labs(
    title = "Effect of Heteroskedasticity on Coverage",
    subtitle = "Conditioned: corr=IID, error=gaussian. Sandwich and GAULSS should be robust."
  )
```
...
```{r hetero-effect-table}
make_effect_table(hetero_summary, "hetero_f", "Heteroskedasticity")
```

Under IID Gaussian, heteroskedasticity has negligible impact: all methods
maintain ~90–91% coverage regardless of the bump pattern. This aligns with
the ANOVA finding that no factor has eta² > 1% in this regime. The sandwich
methods (including GAULSS) are designed for this, but the default also copes
well because the Bayesian posterior variance adapts to local noise levels.

## Error Distribution Effect {.tabset}

**Conditioned on**: corr = IID, hetero = none (clean baseline).
Shows: pffr, pffr_sandwich, pffr_hc.

### Coverage by Error Distribution
...
error_summary <- d |>
  ...
  summarize_coverage(method, term_type, error_f)

plot_parameter_effect(error_summary, "error_f", "Error Distribution") +
  labs(
    title = "Effect of Error Distribution on Coverage",
    subtitle = "Conditioned: corr=IID, hetero=none. t(6) has heavier tails than Gaussian."
  )
```
...
```{r error-effect-table}
make_effect_table(error_summary, "error_f", "Error Distribution")
```

Under the clean baseline (IID, no heteroskedasticity), the error distribution
has virtually no effect: Gaussian and t(6) give identical coverage (~91% for
pffr, ~88% for sandwich, ~90% for HC). The heavier tails of t(6) do not
degrade the Bayesian CIs under these well-specified conditions.

## SNR Effect {.tabset}

**Conditioned on**: well-specified conditions per method. Each method
is shown only under DGP settings where it is appropriate.

### Coverage by SNR

```{r snr-effect-plot}
# Well-specified subset: each method under its well-specified conditions.
# Note: methods are conditioned on different DGP populations (e.g., pffr on
# IID only, pffr_ar on correlated only), so cross-method comparisons of
# parameter effects should be interpreted with this caveat.
d_wellspec <- bind_rows(
  d |> dplyr::filter(
    method %in% c("pffr", "pffr_sandwich", "pffr_hc"),
    corr_type == "iid", hetero_type == "none", error_dist == "gaussian"
  ),
  d |> dplyr::filter(
    method == "pffr_gaulss",
    corr_type == "iid", error_dist == "gaussian"
  ),
  d |> dplyr::filter(
    method == "pffr_ar",
    corr_type %in% c("ar1", "fourier_pos"),
    hetero_type == "none", error_dist == "gaussian"
  )
)

snr_summary <- d_wellspec |>
  summarize_coverage(method, term_type, snr_f)

plot_parameter_effect(snr_summary, "snr_f", "Signal-to-Noise Ratio") +
  labs(
    title = "Effect of SNR on Coverage",
    subtitle = "Each method under its well-specified DGP conditions."
  )
```
...
Under well-specified conditions, higher SNR slightly *decreases* coverage for
all methods (1–3pp). The effect is modest — the ANOVA confirmed SNR is not
a major driver of SE calibration. The pffr_ar method shows the largest
SNR sensitivity (~87% at low vs ~85% at high), suggesting that the AR
correlation correction interacts with signal strength.

## Sample Size Effect {.tabset}

### Coverage by Sample Size

```{r n-effect-plot}
n_summary <- d_wellspec |>
  summarize_coverage(method, term_type, n_f)
...
### Sample Size Effect Table

```{r n-effect-table}
make_effect_table(n_summary, "n_f", "Sample Size")
```

Sample size has opposite effects depending on method. The sandwich methods
improve with n (pffr_sandwich: 87→89%; pffr_ar: 84→88%) as expected from
asymptotic theory. Default and HC are already near nominal at n=50 and
remain stable. This confirms that the sandwich requires sufficient
clusters for reliable inference.

## Wiggliness Effect {.tabset}

### Coverage by Wiggliness

```{r wig-effect-plot}
wig_summary <- d_wellspec |>
  summarize_coverage(method, term_type, wiggliness_f)

plot_parameter_effect(wig_summary, "wiggliness_f", "Wiggliness") +
  labs(
    title = "Effect of Wiggliness on Coverage",
    subtitle = "Each method under its well-specified DGP conditions. Low wiggliness can cause smoothing bias."
  )
```
...
Higher wiggliness slightly degrades coverage for the sandwich methods
(pffr_sandwich: 89→87%; pffr_gaulss: 91→89%), while default and HC are
essentially unaffected. The wiggly truth functions require more basis
functions, which may strain the sandwich variance estimator by increasing
the effective number of parameters relative to the sample size.
...
Marginal performance averaged across all DGPs. pffr_sandwich, pffr_gaulss,
and pffr_ar achieve ~85–87% average coverage, outperforming default pffr
(78%) and pffr_hc (77%). Caution: these marginals mix well-specified and
misspecified conditions, so they understate method performance under the
right conditions (see well-specified comparisons below).

## Performance by Term Type

```{r method-by-term, fig.height = 6}
ggplot(
  global_summary,
  aes(x = method, y = mean_coverage, color = term_type, shape = term_type)
) +
...
```{r misspec-heatmap, fig.height = 10, fig.width = 12}
heatmap_data <- d |>
...
```{r width-coverage-tradeoff, fig.height = 7}
tradeoff_data <- d |>
  group_by(method, term_type) |>
  summarize(
...
# Non-Gaussian Data: Sandwich Coverage
...
- **cl2**: bias-corrected cluster sandwich (CR2/CL2 leverage correction)
...
# Redefine here in case non-Gaussian section is rendered standalone
if (!exists("PRACTICAL_THRESHOLD")) PRACTICAL_THRESHOLD <- 0.05
```


## Non-Gaussian: Data Loading


```{r s1-load-data}
s1_dir <- ci_path("ci-benchmark/study1-nongaussian")
s1_cl2_dir <- ci_path("ci-benchmark/study1-nongaussian-cl2")
s1_files <- list.files(
  s1_dir,
...
cat("Non-Gaussian data loaded:", nrow(s1), "rows\n")
...
s1_summary |>
  ...
  gt() |>
  tab_header(
    title = "Non-Gaussian: Coverage by Family × Correlation × Method",
  ...
  tab_spanner(label = "CL2", columns = starts_with("cl2_"))
```

The coverage table confirms the main non-Gaussian patterns. Under IID:
Poisson default undercovers (73%) due to unmodeled overdispersion, while
HC (89%) and cluster (88%) correct it. Binomial default is adequate (87%).
Under AR1: default coverage collapses to 45–56%, HC to 55–60%, but
cluster maintains 86–89%. The cluster sandwich is essential under
within-curve correlation regardless of family.

## Non-Gaussian: Coverage Heatmap

White = nominal (90%). Red = undercoverage. Blue = overcoverage.


```{r s1-heatmap, fig.width = 12, fig.height = 8}
s1_summary |>
  ggplot(aes(
    x = method,
  ...
  labs(
    title = "Non-Gaussian: Coverage Heatmap",
...
## Non-Gaussian: Coverage Improvement (cluster − default)
...
```{r s1-paired-diff, fig.width = 12, fig.height = 7}
...
ggplot(
  ...
  labs(
    title = "Non-Gaussian: Coverage Improvement (cluster − default)",
...
## Non-Gaussian: Three-Way Method Comparison
...
```{r s1-method-facet, fig.width = 12, fig.height = 8}
s1_summary |>
  ggplot(aes(
    x = corr_f,
    y = coverage,
  ...
  labs(
    title = "Non-Gaussian: Coverage by Correlation Structure",
...
## Non-Gaussian: SE Calibration (z_sd)
...
```{r s1-zsd, fig.width = 12, fig.height = 8}
s1_summary |>
  ...
  scale_color_brewer(palette = "Set1") +
  labs(
    title = "Non-Gaussian: SE Calibration (z_sd, target = 1)",
...
## Non-Gaussian: HC vs Cluster Under Correlation
...
```{r s1-hc-vs-cluster, fig.width = 12, fig.height = 7}
s1_trio <- s1 |>
...
ggplot(s1_trio_long, aes(x = corr_f, y = gain_pp, fill = comparison)) +
  ...
  labs(
    title = "Non-Gaussian: Coverage Gain Over Default by Correlation",
...
## Non-Gaussian: CL2 vs Cluster (Bias-Corrected Sandwich)

CL2 applies a CR2-type leverage correction to the cluster sandwich. At
G=200-400 (Study 1), the per-cluster leverage is tiny (trace(H_gg) ≈ 0.22),
so the CL2 correction should be negligible — unlike Study 2 where G=20-80
and CL2 gains 5-10pp. This section verifies.


```{r s1-cl2-vs-cluster, fig.width = 12, fig.height = 8}
if ("cl2" %in% levels(s1$method)) {
  s1_cl2_comparison <- s1_summary |>
    filter(method %in% c("cluster", "cl2")) |>
  ...
  ggplot(
    ...
    labs(
      title = "Non-Gaussian: CL2 vs Cluster Coverage",
      subtitle = "G=200-400. CL2 correction expected to be negligible at this cluster count.",
      x = "Correlation",
      y = "Coverage",
      color = "Method"
    ) +
...
```
```{r s1-cl2-cluster-diff}
if ("cl2" %in% levels(s1$method)) {
  s1_cl2_diff <- s1_summary |>
    filter(method %in% c("cluster", "cl2")) |>
    select(family_f, corr_f, n_f, method, term_type, coverage) |>
    pivot_wider(names_from = method, values_from = coverage) |>
    mutate(diff_pp = (cl2 - cluster) * 100)

  cat("CL2 − cluster coverage difference (pp):\n")
  s1_cl2_diff |>
    arrange(family_f, corr_f, n_f, term_type) |>
    mutate(
      cl2_fmt = sprintf("%.1f%%", cl2 * 100),
      cluster_fmt = sprintf("%.1f%%", cluster * 100),
      diff_fmt = sprintf("%+.1fpp", diff_pp)
    ) |>
    select(
      family_f, corr_f, n_f, term_type,
      cluster_fmt, cl2_fmt, diff_fmt
    ) |>
    print(n = 50)

  cat(sprintf(
    "\nMean |CL2 − cluster| = %.2fpp (expected ≈ 0 at G=200-400)\n",
    mean(abs(s1_cl2_diff$diff_pp))
  ))
}
```


## Non-Gaussian: Poisson IID — The Overdispersion Story
...
```{r s1-iid-family-comparison, fig.width = 10, fig.height = 6}
s1_iid <- s1_summary |>
  filter(corr_f == "IID")
...
ggplot(s1_iid, aes(x = method, y = coverage, fill = family_f)) +
  ...
  coord_cartesian(ylim = c(0.6, 1)) +
  labs(
    title = "Non-Gaussian: IID Coverage — Poisson vs Binomial",
...
```{r s1-width, fig.width = 12, fig.height = 7}
s1_summary |>
  ...
  labs(
    title = "Non-Gaussian: Mean CI Width by Method and Correlation",
...
## Non-Gaussian: Width vs Coverage Tradeoff


```{r s1-width-coverage, fig.width = 10, fig.height = 7}
s1_summary |>
  ...
  scale_color_manual(values = COLORS_S1) +
  labs(
    title = "Non-Gaussian: Coverage vs CI Width Tradeoff",
...
## Non-Gaussian: Sanity Checks


```{r s1-sanity}
cat("=== Non-Gaussian Sanity Checks ===\n\n")
...
### CL2 at G=200-400: negligible correction (as expected)

The CL2 bias-corrected sandwich gives essentially the same coverage as
plain cluster at G=200-400 (mean |CL2 − cluster| < 1pp). This is
consistent with the theoretical expectation: per-cluster leverage is
O(p/G), which at G=200 and ~144 basis functions gives trace(H_gg) ≈ 0.22
— a ~1% SE inflation that has negligible coverage impact.

This contrasts sharply with Study 2 (G=20-80) where CL2 gains 5-10pp.
The recommendation is clear: CL2 matters at small G but is unnecessary
at G ≥ 200.
...
1. ~~**CR2/CR3 sandwich**~~: Done — CL2 results confirm negligible
   correction at G=200-400. The Binomial IID ff issue persists with CL2,
   so it is not a finite-sample sandwich bias problem.
...
**Study 2 primary question:** As the response grid becomes finer (more evaluation
points per function), does the default Bayesian covariance remain calibrated under
within-curve correlation, or does the cluster-robust sandwich provide meaningfully
better coverage? Does grid refinement uniformly benefit or uniformly harm coverage?

**Central finding (preview):** Under IID errors, default and CL2 coverage are near
nominal (~89-90%); cluster undercovers at ~82-86%. Under AR1(0.9) or Fourier+(0.3)
correlation, **default SE calibration deteriorates dramatically with finer grids** —
coverage of the ff term drops from ~62% (coarse) to ~43% (fine) while z_sd rises
from ~1.9 to ~3.2. The cluster sandwich is stable across grid resolutions but
remains well below the 90% nominal, with strong dependence on n (56% at n=20,
82% at n=80 under AR1). HC (observation-level only) behaves like default under
correlation. **CL2 (bias-corrected cluster) consistently outperforms cluster by
5-10pp** at these cluster sizes (G=20-80), approaching nominal under IID.

The grid refinement study holds nxgrid fixed at 60 to isolate the response-grid
(nygrid) dimension, which is most relevant for the CL2 computational cost.
...
- **Full model**: ff + linear + smooth + concurrent (6 term types incl. intercept and E(Y))

- **Main grid factor**: nygrid in {40, 80, 120} with **nxgrid fixed at 60**

- **4 methods**: default, cluster, HC, CL2 (all have full coverage results)

- **50 reps** per DGP x grid cell
...
```{r s2-summary}
# s2_summary_by_n: keeps n and snr as separate dimensions for n-effect plots.
s2_summary_by_n <- s2 |>
...
# s2_summary: aggregates over n, keeping snr × corr × grid breakdown.
# Re-aggregate from raw data (not from s2_summary_by_n) for valid MC SEs.
s2_summary <- s2 |>
  dplyr::filter(!is.na(coverage)) |>
...
Coverage and SE calibration (z_sd) for the ff term by n × grid × correlation.
All 4 methods shown (default, cluster, HC, CL2). SNR averaged (confirmed
negligible effect — see heatmap).

Key patterns to look for:

- **IID**: default and CL2 near 90%; cluster undercovers at ~82-86%
  (t vs z critical value with small G). Grid resolution has minimal effect.

- **AR1/Fourier+**: default coverage collapses as nygrid increases (SE deflation,
  z_sd >> 1). Cluster coverage is stable across grids but well below nominal,
  with strong n-dependence (56% at n=20, 82% at n=80 under AR1).

- **HC** behaves like default under correlation — corrects heteroskedasticity
  but not within-curve dependence.

- **CL2** consistently outperforms cluster by 5-10pp at these cluster sizes
  (G=20-80); the CR2 leverage correction is substantial with few clusters.

```{r s2-summary-table, results='asis'}
s2_cov_by_term <- s2_summary_by_n |>
  # Average over SNR (negligible effect)
...
make_s2_coverage_table <- function(data, term) {
  wide <- data |>
    dplyr::filter(term_type == term) |>
    select(n_f, corr_f, grid_f, method, coverage, mc_se, z_sd) |>
    mutate(
      coverage_fmt = fmt_coverage(coverage, mc_se),
      z_sd_fmt = sprintf("%.2f", z_sd)
    ) |>
    select(n_f, corr_f, grid_f, method, coverage_fmt, z_sd_fmt) |>
    pivot_wider(
      names_from = method,
      values_from = c(coverage_fmt, z_sd_fmt),
      names_glue = "{method}_{.value}"
    ) |>
    arrange(n_f, corr_f, grid_f)

  # Build cols_label and tab_spanner dynamically based on available methods
  available_methods <- unique(data$method)
  method_labels <- list(
    default = "default", cluster = "cluster", hc = "HC", cl2 = "CL2"
  )

  label_args <- list(n_f = "n", corr_f = "Correlation", grid_f = "Grid (nygrid)")
  for (m in names(method_labels)) {
    if (m %in% available_methods) {
      cov_col <- paste0(m, "_coverage_fmt")
      zsd_col <- paste0(m, "_z_sd_fmt")
      if (cov_col %in% names(wide)) label_args[[cov_col]] <- "Coverage"
      if (zsd_col %in% names(wide)) label_args[[zsd_col]] <- "z_sd"
    }
  }
  ...
  tbl <- wide |>
    gt() |>
    tab_header(
      title = paste("Study 2:", term, "Coverage by n \u00d7 Grid \u00d7 Correlation \u00d7 Method"),
      subtitle = "Coverage % (\u00b195% MC interval) and z_sd (target = 1.0); SNR averaged"
    ) |>
    cols_label(.list = label_args)

  for (m in names(method_labels)) {
    if (m %in% available_methods && any(startsWith(names(wide), paste0(m, "_")))) {
      tbl <- tbl |>
        tab_spanner(label = method_labels[[m]], columns = starts_with(paste0(m, "_")))
    }
  }
  tbl
}

s2_terms <- levels(s2$term_type)
s2_terms <- s2_terms[s2_terms %in% unique(s2_cov_by_term$term_type)]
for (tt in s2_terms) {
  cat(as.character(gt::as_raw_html(make_s2_coverage_table(s2_cov_by_term, tt))))
  cat("\n\n")
}
```
...
```{r s2-heatmap, fig.width = 14, fig.height = 10}
```

One heatmap per term type. Rows = SNR, columns = nygrid.
Red = under-coverage; blue = over-coverage; white = nominal (90%).

```{r}
s2_heatmap_data <- s2_summary_by_n |>
  dplyr::filter(
    term_type %in%
      c("intercept", "ff", "linear", "smooth", "concurrent", "E(Y)"),
    method %in% c("default", "cluster", "hc", "cl2")
  ) |>
  mutate(
    y_label = interaction(corr_f, paste0("n=", n_f), sep = " | ")
  )

s2_heatmap_terms <- levels(s2_heatmap_data$term_type)
s2_heatmap_terms <- s2_heatmap_terms[
  s2_heatmap_terms %in% unique(s2_heatmap_data$term_type)
]

for (tt in s2_heatmap_terms) {
  p <- s2_heatmap_data |>
    dplyr::filter(term_type == tt) |>
    ggplot(aes(x = method, y = y_label, fill = coverage)) +
    geom_tile(color = "white", linewidth = 0.5) +
    geom_text(aes(label = sprintf("%.0f%%", coverage * 100)), size = 3.5) +
    scale_fill_gradient2(
      low = "#d73027",
      mid = "white",
      high = "#4575b4",
    ...
    facet_grid(snr_f ~ grid_f) +
    labs(
      title = paste("Study 2 Coverage:", tt),
      subtitle = "nxgrid fixed at 60. Red = undercoverage, blue = overcoverage.",
      x = "Method",
      y = "Correlation | n",
      fill = "Coverage"
    ) +
...
The expanded heatmap now shows each n level separately. Key patterns visible:
under IID, coverage is stable across grid levels for all methods, with default
and CL2 near 90% and cluster at 82–86%. Under AR1/Fourier+, coverage depends
strongly on both n and grid: default worsens on finer grids while cluster
improves with n. The n=80 + CL2 combination approaches nominal under all
correlation structures.

## Study 2: Key Finding — Default SE Collapse on Finer Grids (Hero Figure)

**Central result of Study 2.** Under within-curve correlation (AR1, Fourier+), default
SEs shrink relative to true estimation uncertainty as the response grid becomes finer.
This is because each function is evaluated at more grid points that are highly correlated,
but the default Bayesian covariance ignores within-curve dependence.
The cluster-robust sandwich remains comparably stable across grid resolutions.
Under IID errors, all methods are stable across grid resolutions (grid refinement has
negligible effect), though coverage levels differ: default and CL2 near 90%, cluster ~82-86%.

HC (observation-level heteroskedasticity-robust sandwich) behaves like default under
correlation — it corrects heteroskedasticity but not within-curve dependence.

```{r s2-hero-ff, fig.width = 14, fig.height = 12}
```

**Hero figure**: ff term coverage by nygrid, showing all 4 methods and all 3 sample
sizes. SNR averaged (confirmed negligible). Each column = one sample size; each
row = one correlation structure.

Key observations:

- **IID** (top row): default and CL2 near 90% at all n and grid levels;
  cluster undercovers at ~82-86%; HC matches default.

- **AR1/Fourier+** (middle, bottom): default collapses dramatically with finer grids
  (from ~62% coarse to ~43% fine). Cluster is stable across grids but well
  below nominal, with strong n-dependence (n=20: ~56%, n=80: ~82% under AR1).
  HC tracks default — corrects heteroskedasticity but not within-curve dependence.

- **CL2 vs cluster**: CL2 consistently outperforms cluster by 5-10pp. At G=20-80,
  the CR2 leverage correction produces substantial SE inflation that compensates
  for finite-sample sandwich bias.
...
s2_summary_by_n |>
  ...
  labs(
    title = "Study 2: ff Coverage by nygrid — Default SE Collapse under Correlation",
    subtitle = paste0(
      "Red dashed = 90% nominal. SNR averaged (negligible effect). nxgrid fixed at 60.\n",
      "All 4 methods shown. CL2 outperforms cluster by 5-10pp at these cluster sizes."
    ),
...
## Study 2: Coverage by Grid Level — All Term Types (n shown explicitly)

Full grid x correlation x n breakdown averaged over SNR (SNR barely matters --
confirmed in the hero figure and heatmap). All 6 term types and all 4 methods
shown. The grid effect is most pronounced for ff and concurrent under
AR1/Fourier+ correlation. Linear and smooth terms show milder effects.
Intercept shows a coarse-grid artifact (see Intercept Diagnostics below).
E(Y) (overall prediction) reflects the composite signal quality.

```{r s2-coverage-by-grid, fig.width = 16, fig.height = 24}
# Average over SNR (negligible effect)
s2_summary_by_n |>
  group_by(n_f, corr_f, grid_f, method, term_type) |>
  summarize(
  ...
  facet_grid(corr_f + n_f ~ term_type) +
  scale_y_continuous(labels = scales::percent, limits = c(0.2, 1)) +
  scale_color_manual(values = COLORS_S2) +
  labs(
    title = "Study 2: Coverage by nygrid — All Terms, All Methods (SNR averaged)",
    subtitle = paste0(
      "Red dashed = 90% nominal. 9 rows = 3 corr \u00d7 3 n; 6 columns = all term types.\n",
      "All 4 methods shown. nxgrid fixed at 60."
    ),
    x = "Response Grid (nygrid)",
    y = "Coverage",
    color = "Method"
...
Intercept and E(Y) excluded (intercept has a separate grid-dependent artifact;
E(Y) is a composite — see Intercept Diagnostics below).


```{r s2-paired-diff, fig.width = 12, fig.height = 7}
...
plot_s2_paired_diff(
  ...
  subtitle = paste0(
    "Positive = cluster helps. Averaged over n = 20, 40, 80; nxgrid fixed at 60.\n",
    "Under IID, cluster may underperform (negative diff = cluster undercovers relative to default).\n",
    "Under AR1/Fourier+, cluster gain should INCREASE with finer grids (default worsens)."
  )
)
```
...
```{r s2-paired-diff-cl2, fig.width = 12, fig.height = 7}
if (sum(as.character(s2$method) == "cl2", na.rm = TRUE) == 0) {
  ...
  if (nrow(s2_paired_summary_cl2) == 0) {
    ...
    plot_s2_paired_diff(
      ...
      subtitle = "Positive = CL2 improves coverage over cluster. Averaged over n; nxgrid fixed at 60."
    )
  }
}
```
...
```{r s2-zsd-by-grid, fig.width = 16, fig.height = 30}
```

z_sd > 1 means SEs are too small (anti-conservative = undercoverage).
**Expected pattern**: Under IID, z_sd ≈ 1 for default and ≈ 0.9 for cluster (SEs slightly
oversized on the SE-calibration scale, z_sd < 1). Note: z_sd < 1 does NOT imply overcoverage
— cluster uses z = 1.96 critical value while finite-sample cluster sandwich follows t_(G−1);
with G = 20–80 this t vs z gap produces 83–85% coverage despite well-sized SEs.
Under AR1/Fourier+, default z_sd should INCREASE sharply with finer
grids (more anti-conservative), while cluster z_sd remains moderate (1.5-1.8).
HC tracks default; CL2 has z_sd similar to cluster but consistently higher coverage
(the CR2 correction inflates SEs, bringing CL2 closer to nominal).
This mechanistically explains the coverage collapse for default.

```{r}
s2_summary_by_n |>
  # Average over SNR (negligible effect)
  group_by(n_f, corr_f, grid_f, method, term_type) |>
  summarize(z_sd = mean(z_sd, na.rm = TRUE), .groups = "drop") |>
  ggplot(aes(
    x = grid_f,
    y = z_sd,
    color = method,
  ...
  facet_grid(corr_f + n_f ~ term_type, scales = "free_y") +
  scale_color_manual(values = COLORS_S2) +
  labs(
    title = "Study 2: SE Calibration (z_sd) by nygrid — All Methods, All Terms (SNR averaged)",
    subtitle = paste0(
      "Free y-axis per panel. z_sd > 1 = SEs too small (undercoverage).\n",
      "9 rows = 3 corr \u00d7 3 n; 6 columns = all term types. All 4 methods shown."
    ),
    x = "Response Grid (nygrid)",
    y = "z_sd",
    color = "Method"
...
```{r s2-intercept-bias, fig.width = 14, fig.height = 8}
```

Mechanistic breakdown of the intercept undercoverage on coarse grids.
The paradox: per-rep z_mean shows alternating spatial bias (canceling in the rep-average),
while sd(z_mean) across reps reveals large rep-to-rep global shifts.
Result: individual reps have systematic intercept offsets, but the average offset is near
zero. Coverage fails because individual CIs miss the true intercept, even though the
average miss is zero.

**Note on SNR**: The coarse-grid artifact is particularly severe at high SNR (SNR = 25).
At high SNR the model fits precisely, making it sensitive to discretization bias, which
manifests as large rep-to-rep variation in z_mean.

```{r}
s2_intercept <- s2_summary |>
  dplyr::filter(term_type == "intercept")

# Numeric sanity check: print z_sd per grid/corr/snr for cluster intercept
cat("Cluster intercept z_sd by grid and SNR (averaged over n):\n")
s2_intercept |>
  dplyr::filter(method == "cluster") |>
  select(snr_f, corr_f, grid_f, coverage, z_sd, z_mean_sd) |>
  arrange(snr_f, corr_f, grid_f) |>
  print(n = 30)
...
p_zmean_sd <- ggplot(
  ...
  scale_color_manual(values = COLORS_S2) +
  labs(
    title = "Intercept: Between-Rep SD of z_mean (Pooled across n)",
    subtitle = paste0(
      "sd(z_mean across reps) per (SNR, corr, grid) cell.\n",
      "Large values = systematic rep-to-rep intercept shifts. Especially severe at SNR = 25, coarse grid."
    ),
    x = "Response Grid (nygrid)",
...
p_zsd <- ggplot(
  ...
  scale_color_manual(values = COLORS_S2) +
  labs(
    title = "Intercept z_sd by Grid Resolution (Averaged over n)",
    subtitle = paste0(
      "Within-rep z_sd (target = 1). All methods show z_sd > 1 \u2014 SEs underestimate\n",
      "true variability for the intercept, especially under correlation."
    ),
    x = "Response Grid (nygrid)",
    y = "z_sd",
    color = "Method"
...
p_cov <- ggplot(
  ...
  scale_color_manual(values = COLORS_S2) +
  labs(
    title = "Intercept Coverage by Grid Resolution (Averaged over n)",
    subtitle = paste0(
      "nxgrid fixed at 60. All 4 methods shown. Intercept coverage below nominal for all;\n",
      "the coarse-grid artifact (SNR = 25, IID) is especially striking (see sd(z_mean))."
    ),
    x = "Response Grid (nygrid)",
    y = "Coverage",
    color = "Method"
...
```
```{r s2-intercept-zsd, fig.width = 14, fig.height = 8}
p_zsd

```
```{r s2-intercept-cov, fig.width = 14, fig.height = 8}
p_cov
```
...
```{r s2-cov-quality, fig.width = 12, fig.height = 14}
```

**Diagonal variance ratio = mean(model SE²) / Var(est − truth).**
Target = 1. Values < 1 mean model SEs systematically underestimate true
estimation uncertainty. This is the mechanistic heart of the default SE collapse:
as the response grid becomes finer, more correlated evaluations per curve
are added, increasing true Var(est) while model SE² stays roughly constant
(or shrinks), driving the ratio below 1.

**Expected pattern**:

- IID: all methods ≈ 1 on all grids (well-calibrated SEs).

- AR1/Fourier+: default ratio **decreases** with finer grids; cluster ratio
  remains elevated (~0.7–1.0) or somewhat stable.

Bars show IQR of per-cell median(ratio) across n values
(n = 20, 40, 80 = 3 cells per bar; SNR is faceted out).

```{r}
if (is.null(s2_cov)) {
  stop(
    "Covariance quality file not found at: ",
    s2_cov_file,
    "\nThis file is required for the diagonal ratio analysis."
  )
}
...
# Aggregate over n for a cleaner plot (snr × corr × grid retained)
...
ggplot(
  ...
  scale_color_manual(values = COLORS_S2) +
  labs(
    title = "Study 2: Diagonal Variance Ratio by nygrid (Averaged over n)",
    subtitle = paste0(
      "Cell-level median of mean(SE\u00b2) / Var(est \u2212 truth); bars = IQR across n (3 values per bar).\n",
      "Target = 1.0. nxgrid fixed at 60. CL2 outliers at n=20/Fourier+ clipped (y \u2264 2.5).\n",
      "Under correlation, default ratio drops with finer grids; cluster remains closer to 1."
    ),
    x = "Response Grid (nygrid)",
    y = "Diagonal Ratio",
    color = "Method"
...
**Note**: Intercept excluded (separate grid-dependent artifact; see Intercept
Diagnostics). E(Y) not available in the covariance quality data.
CL2 at n = 20 under Fourier+ has extreme ratios (> 1000) due to unstable
sandwich with very few effective clusters — clipped to y ∈ [0, 2.5] for readability.

## Study 2: CI Width by Grid


```{r s2-width, fig.width = 16, fig.height = 30}
```

**CI width vs coverage tradeoff.** All 4 methods and all 6 term types shown,
stratified by n and correlation (SNR averaged since negligible effect).

Key patterns:

- Under IID: cluster CIs are wider than default but have lower coverage
  (~82-86% vs ~90%). CL2 CIs are wider still and reach ~89-90%.

- Under AR1/Fourier+: cluster and CL2 CIs are substantially wider (correct response
  to correlation). CL2 achieves the best coverage; cluster still undercovers.
  Default/HC CIs are narrow and severely undercover.

- On finer grids under correlation: cluster CI width grows (reflecting increasing
  per-curve correlation), while default CI width stays roughly constant.
...
s2_summary_by_n |>
  ...
  scale_color_manual(values = COLORS_S2) +
  labs(
    title = "Study 2: Mean CI Width by nygrid — All Methods, All Terms (SNR averaged)",
    subtitle = paste0(
      "9 rows = 3 corr \u00d7 3 n; 6 columns = all term types. All 4 methods shown.\n",
      "Free y-axis per row (corr \u00d7 n combinations have different width ranges)."
    ),
    x = "Response Grid (nygrid)",
    y = "Mean CI Width",
    color = "Method"
...
## Study 2: n Effect on Coverage under Correlation (Fine Grid)

Under correlated errors, cluster coverage at the fine grid depends strongly on n.
For small n = 20, the cluster sandwich operates with relatively few independent
units (20 curves), so the sandwich approximation may be coarse. Coverage improves
substantially as n increases from 20 → 40 → 80.

```{r s2-n-effect, fig.width = 12, fig.height = 7}
```

All 4 methods, ff term at the fine grid (ny = 120) by n and corr_type.
Under IID, default and CL2 near 90% for all n; cluster ~82-86%.
Under AR1/Fourier+, cluster improves from ~57% to ~82%, and CL2 from ~65% to ~89%,
approaching nominal. This suggests that at n = 80, CL2 nearly closes the coverage gap.
With few curves (n = 20), even the sandwich provides little protection beyond the default.
...
s2_summary_by_n |>
  ...
  geom_point(size = 3, position = position_dodge(width = 0.3)) +
  geom_line(linewidth = 0.9, position = position_dodge(width = 0.3)) +
  geom_errorbar(
    aes(
      ymin = coverage - 1.96 * mc_se,
      ymax = coverage + 1.96 * mc_se
  ...
  facet_wrap(~corr_f, nrow = 1) +
  scale_y_continuous(labels = scales::percent, limits = c(0.3, 1)) +
  scale_color_manual(values = COLORS_S2) +
  labs(
    title = paste0(
      "Study 2: ff Coverage vs n at Fine Grid (nygrid = ",
      max(S2_EXPECTED_NYGRID),
      ") — All Methods (SNR averaged)"
    ),
    subtitle = paste0(
      "Red dashed = 90% nominal. All 4 methods shown.\n",
      "Under AR1/Fourier+, cluster and CL2 improve sharply with n; CL2 outperforms cluster by 5-10pp."
    ),
    x = "Sample size (n = number of curves)",
    y = "Coverage",
    color = "Method"
  ) +
  theme(
...
Note: CL2 timing depends on nygrid and n (O(G × p²) where p ∝ nygrid),
but does NOT depend on the correlation structure of the data.


```{r s2-cl2-inference-overhead, fig.width = 12, fig.height = 9}
s2_cl2_timing_dir <- ci_path("ci-benchmark/study2-cl2-timing")
...
s2_cl2_nx_side_dir <- ci_path("ci-benchmark/study2-nx-sidecheck")
...
```{r s2-cl2-inference-overhead-nx-sidecheck, fig.width = 11, fig.height = 6}
...
s2_fine_diff <- s2_summary |>
  ...
  select(snr_f, corr_f, term_type, method, coverage) |>
...
if (nrow(s2_fine_diff) > 0) {
  cat("\nFine-grid coverage gain (cluster - default) by SNR and corr_type:\n")
...
Findings are stratified by correlation type and SNR to avoid masking critical
...
cat("\n2) Fine-grid (ny =", max(S2_EXPECTED_NYGRID), ")")
cat(" cluster vs default coverage — ff term, by corr_type and SNR:\n")
fine_ff_table <- s2_summary |>
  dplyr::filter(
    term_type == "ff",
    grid_f == s2_fine_display,
    method %in% c("default", "cluster", "cl2")
  ) |>
  select(snr_f, corr_f, method, coverage) |>
  tidyr::pivot_wider(names_from = method, values_from = coverage) |>
  mutate(diff = cluster - default) |>
  arrange(snr_f, corr_f)

if (nrow(fine_ff_table) == 0) {
  cat("   Not available in the current extract.\n")
} else {
  apply(fine_ff_table, 1, function(row) {
    cl2_str <- if ("cl2" %in% names(row) && !is.na(row[["cl2"]])) {
      paste0(", cl2=", s2_fmt_pct(as.numeric(row[["cl2"]])))
    } else {
      ""
    }
    cat(
      sprintf(
        "   %s, %s: default=%s, cluster=%s%s, cluster-default=%s\n",
        row[["snr_f"]],
        row[["corr_f"]],
...
cat(
  "\n3) Grid effect on default ff coverage (coarse vs fine, by corr_type and SNR):\n"
)
grid_effect_table <- s2_summary |>
  dplyr::filter(
    term_type == "ff",
    method == "default",
    grid_f %in% c(s2_coarse_display, s2_fine_display)
  ) |>
  select(snr_f, corr_f, grid_f, coverage) |>
  tidyr::pivot_wider(names_from = grid_f, values_from = coverage) |>
  arrange(snr_f, corr_f)

if (nrow(grid_effect_table) > 0) {
  apply(grid_effect_table, 1, function(row) {
    coarse_val <- as.numeric(row[[s2_coarse_display]])
    fine_val <- as.numeric(row[[s2_fine_display]])
    cat(
      sprintf(
        "   %s, %s: %s (coarse) -> %s (fine), change: %s\n",
        row[["snr_f"]],
        row[["corr_f"]],
        s2_fmt_pct(coarse_val),
        s2_fmt_pct(fine_val),
        s2_fmt_pp(fine_val - coarse_val)
      )
    )
  })
}
...
cluster_intercept_grid <- s2_summary |>
  dplyr::filter(method == "cluster", term_type == "intercept") |>
  group_by(grid_f) |>
  summarize(coverage = mean(coverage, na.rm = TRUE), .groups = "drop")
s2_intercept_coarse <- cluster_intercept_grid |>
  dplyr::filter(grid_f == s2_coarse_display) |>
  pull(coverage) |>
  s2_safe_first()
s2_intercept_fine <- cluster_intercept_grid |>
  dplyr::filter(grid_f == s2_fine_display) |>
  pull(coverage) |>
  s2_safe_first()

cat("\n4) Intercept coverage change from coarsest to finest observed grid\n")
cat(
  sprintf(
    "   cluster intercept (avg over SNR and corr): %s -> %s (%s)\n",
...
cat(
  "\n5) Under IID: default vs cluster at fine grid (ff term, avg over SNR and n):\n"
)
iid_fine <- s2_summary |>
  dplyr::filter(
    term_type == "ff",
  ...
  group_by(method) |>
  summarize(coverage = mean(coverage, na.rm = TRUE), .groups = "drop")
for (i in seq_len(nrow(iid_fine))) {
  cat(sprintf(
    "   %s: %s\n",
    iid_fine$method[i],
    s2_fmt_pct(iid_fine$coverage[i])
  ))
}
cat(
  "   Under IID: default and CL2 hit ~89-90% nominal; cluster ~82-86% (undercoverage).\n"
)
cat("   Grid refinement does not worsen default under IID.\n")
cat(
  "   Likely cause of cluster undercoverage: sandwich z-scores follow t_(G-1) with G = 20-80;\n"
)
cat(
  "   using z = 1.96 instead of t_(G-1) ~ 2.0-2.1 explains part of the gap.\n"
)
cat(
  "   CL2 achieves ~90% despite the same z = 1.96 cutoff — the CR2 SE inflation compensates.\n"
)

cat(
  "\n6) n-effect: cluster ff coverage at finest grid (nygrid=120) under AR1 (avg over SNR):\n"
)
n_effect_table <- s2_summary_by_n |>
  dplyr::filter(
    term_type == "ff",
    corr_f == "AR1(0.9)",
    grid_f == s2_fine_display,
  ...
  summarize(coverage = mean(coverage, na.rm = TRUE), .groups = "drop") |>
  arrange(n_f)
if (nrow(n_effect_table) == 0) {
  cat("   Not available in current extract.\n")
} else {
  for (i in seq_len(nrow(n_effect_table))) {
    cat(sprintf(
      "   n = %s: cluster = %s\n",
      n_effect_table$n_f[i],
      s2_fmt_pct(n_effect_table$coverage[i])
    ))
  }
  cat(
    "   Key takeaway: cluster coverage improves substantially with n under AR1 on fine grids.\n"
  )
  cat(
    "   Practical implication: cluster sandwich requires n >= 80 to reach ~80% under AR1/Fourier+;\n"
  )
  cat(
    "   smaller studies (n=20) see only ~56% coverage even with cluster-robust SEs.\n"
  )
}
```
